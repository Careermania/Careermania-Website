    ...
    if request.method == "POST":
        ...
        try:
            try:
                ...
                if user:
                    return render(request, 'signup.html', {'error': 'User already exist with this email.'})
            except:
                user = User.objects.get(username=username)
                if user:
                    return render(request, 'signup.html', {'error': 'User already exist with this Username.'})
        except:
            if role == "student":
                user = User.objects.create_student(email, username, password)
                user.save()
                send_confirmation_email(request, user)
                return HttpResponse('Verification mail Sent Successfully. Please Click the link to verify the account.')
            if role == "merchant":
...
def merchant_dashboard(request):
    if request.user.is_merchant:
        ...
        return render(request, 'merchant_dashboard.html', context=context)
    return render(request, 'login.html')
...
def add_faculty(request):
    if request.user.is_merchant:
        ...
        if request.method == "POST":
            ...
            description = request.POST['description']
            faculty_image = request.FILE['pic']
